...
if Vcopy(2)+s2(2) - s1(2)  <= 0 
    if s1(2) - s2(2) + V(2) >= 0  % Im2 doesn't go beyond left of Im1
        col = s1(2);
...
if V(2) <= 0    %Im1 to the right of Im2
    if s1(2) + V(2) -s2(2) >= 0  %Im2 doesn't go beyond the very left of Im1
        sj = size(joint);
        if V(1) >= 0    %Im1 above Im2
            joint(1:s1(1), 1:s1(2)) = Im1(1:end, 1:end);
    ...
    if s1(2) + V(2) -s2(2) < 0  %Im2 goes beyond the very left of Im1
        sj = size(joint);
        if V(1) >= 0    %Im1 above Im2
            joint(1:s1(1), sj(2)-s1(1)+1:sj(2)) = Im1(1:end, 1:end);
            joint(V(1)+1:s2(1)+V(1), 1:s1(2)) = Im2(1:end, 1:end);
            vertShift = V(1); 
            horiShift = horiShift;
        end
        if V(1) < 0    %Im1 below Im2
            joint(Va(1)+1:end, sj(2)-s1(1)+1:end) = Im1(1:end, 1:end);
            joint(1:s2(1), 1:s1(2)) = Im2(1:end, 1:end);
            horiShift = horiShift; 
            vertShift = V(1)-1;
        end
%         vertShift = V(1);
